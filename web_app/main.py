...
async def main(page: ft.Page):
    ...
    state = {
        ...
        "camera_status": "カメラの準備をしています..."
    }

    # スキャン画面用のカメラコントロール（画面が切り替わっても同じインスタンスを使い回す）
    scan_camera = fc.Camera(expand=True, preview_enabled=True)
    scan_status_text = ft.Text(state["camera_status"], color=ft.Colors.WHITE, size=13)
    scan_input_area_text = ft.Text(LANG_TEXTS["ja"]["input_area"], color=ft.Colors.WHITE, size=14)
    ...
    # スキャン画面は「毎回新しく組み立て直す」のではなく、この1つのオブジェクトを使い回す。
    # 毎回作り直すと、中のCameraコントロールごと作り直されたとFlutter側に判断され、
    # 初期化済みの状態が失われてしまうため。
    scan_screen_container = ft.Stack([
        ...
        ft.Container(
            ...
            content=ft.Column([
                ...
                ft.Button(
                    content=ft.Text("撮影してテスト", color=ft.Colors.WHITE),
                    on_click=lambda _: page.run_task(handle_take_photo_async),
                    width=float("inf"),
                ...
                jan_input,
                ft.Button(
                    content=scan_search_btn_text,
                    on_click=lambda _: page.run_task(handle_barcode_search_async, jan_input.value),
                    width=float("inf"),
                    style=ft.ButtonStyle(bgcolor=BRAND_GREEN)
                )
            ], alignment=ft.MainAxisAlignment.START, spacing=15, scroll=ft.ScrollMode.AUTO, expand=True)
        )
    ], expand=True)
    ...
    async def handle_open_scan_async():
        # 1. まず「カメラ初期化中」の状態で画面を切り替える
        state["current_screen"] = "scan_widget"
        state["camera_initialized"] = False
        state["camera_status"] = "カメラを初期化しています..."
        await refresh_ui_async()

        # 2. 利用可能なカメラを取得して初期化（失敗してもタイムアウトせず必ずエラー表示に落ち着かせる）
        try:
            cameras = await scan_camera.get_available_cameras()
            if not cameras:
                state["camera_status"] = "利用可能なカメラが見つかりませんでした。カメラの権限を確認してください。"
                await refresh_ui_async()
                return
            ...
            chosen = cameras[0]
            for c in cameras:
                if getattr(c, "lens_direction", None) == getattr(fc.CameraLensDirection, "BACK", None):
                    chosen = c
                    break

            await scan_camera.initialize(
                description=chosen,
                resolution_preset=fc.ResolutionPreset.MEDIUM,
                enable_audio=False,
            )
            state["camera_initialized"] = True
            state["camera_status"] = "カメラの準備ができました。バーコードを画面に映してください。"
        except Exception as e:
            state["camera_status"] = f"カメラの初期化に失敗しました: {e}"

        await refresh_ui_async()

    async def handle_take_photo_async():
        if not state.get("camera_initialized"):
            state["camera_status"] = "カメラがまだ準備できていません。少し待ってから試してください。"
            await refresh_ui_async()
            return
        try:
            photo_bytes = await scan_camera.take_picture()
            state["camera_status"] = f"撮影しました（{len(photo_bytes)} bytes）。バーコードの自動読み取りは次のステップで対応します。"
        except Exception as e:
            state["camera_status"] = f"撮影に失敗しました: {e}"
        await refresh_ui_async()
    ...
    async def refresh_ui_async():
        ...
        if target == "main":
            ...
            scan_status_text.value = state["camera_status"]
            main_content_area.content = scan_screen_container
    ...
    async def change_screen_async(target_name):
        if target_name == "scan_widget":
            await handle_open_scan_async()
            return
    ...
    async def toggle_language_async(is_english):
        ...
        scan_input_area_text.value = LANG_TEXTS[state["lang"]]["input_area"]
        scan_search_btn_text.value = LANG_TEXTS[state["lang"]]["search_btn"]
        await refresh_ui_async()
...
ft.run(main)async def connect_sheet_async(jan_code: str):
    url = f"{SUPABASE_URL}/rest/v1/products?jan=eq.{jan_code}"
    headers = {"apikey": SUPABASE_ANON_KEY, "Authorization": f"Bearer {SUPABASE_ANON_KEY}"}
...
def get_scan_content(lang, on_search, camera_control, camera_status, on_take_photo):
    t = LANG_TEXTS[lang]
    jan_input = ft.TextField(label="JAN Code", hint_text="e.g. 4901234567890", keyboard_type=ft.KeyboardType.NUMBER)
    ...
    return ft.Stack([
        ...
        ft.Container(
            ...
            content=ft.Column([
                ft.Text("バーコードスキャン", size=22, color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD),
                ft.Text(camera_status, color=ft.Colors.WHITE, size=13),
                ft.Container(
                    height=320,
                    bgcolor=ft.Colors.BLACK,
                    border_radius=10,
                ...
                ft.Button(
                    content=ft.Text("撮影してテスト", color=ft.Colors.WHITE),
                    on_click=lambda _: on_take_photo(),
                    width=float("inf"),
                    style=ft.ButtonStyle(bgcolor=BRAND_GREEN)
                ),
                ft.Divider(height=1, color=ft.Colors.with_opacity(0.3, ft.Colors.WHITE)),
...
async def main(page: ft.Page):
    ...
    async def refresh_ui_async():
        ...
        if target == "main":
            ...
            main_content_area.content = get_scan_content(
                lang,
                lambda jan: page.run_task(handle_barcode_search_async, jan),
                scan_camera,
                state["camera_status"],
                lambda: page.run_task(handle_take_photo_async)
            )
        elif target == "history_widget":
            main_content_area.content = get_history_content(lang, state["history"], lambda e: page.run_task(handle_clear_history_async, e))
        elif target == "settings_widget":
